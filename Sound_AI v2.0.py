...
class VisualizationWindow(tk.Toplevel):
    def __init__(self, master, reduced_df, labels, feature_type, n_clusters_or_labels, segment_length, c_method,
                 score,
        ...
        self.geometry("900x750")  # Increased size
        ...
        # 2. Plot Frame (Top, expands)
        self.plot_frame = tb.Frame(main_frame)
        self.plot_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 5))

        self.fig, self.ax = plt.subplots(figsize=(8, 5))  # Adjusted aspect ratio
        self.canvas = FigureCanvasTkAgg(self.fig, master=self.plot_frame)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

        # 3. Legend Frame (Bottom, fixed height, scrollable)
        tb.Label(main_frame, text="File Legend:", font=("Segoe UI", 9, "bold")).pack(anchor="w", padx=10)

        self.legend_frame = tb.Frame(main_frame, height=150)  # Fixed height for legend area
        self.legend_frame.pack(fill=tk.X, padx=10, pady=10)
        self.legend_frame.pack_propagate(False)  # Force fixed height

        # Scrollbar and Canvas for Legend
        self.legend_scroll = tb.Scrollbar(self.legend_frame, orient="vertical")
        self.legend_scroll.pack(side="right", fill="y")

        self.legend_canvas = tk.Canvas(self.legend_frame, yscrollcommand=self.legend_scroll.set, bg="#f0f0f0")
        self.legend_canvas.pack(side="left", fill="both", expand=True)
        self.legend_scroll.config(command=self.legend_canvas.yview)

        # Frame inside canvas to hold items
        self.legend_inner_frame = tb.Frame(self.legend_canvas)
        self.legend_canvas.create_window((0, 0), window=self.legend_inner_frame, anchor="nw")

        self.legend_inner_frame.bind("<Configure>",
    ...
    def plot_data(self):
        ...
        title = f"{self.c_method} Plot\n"
        if self.has_labels:
            title += f"(N_Labels: {self.n_clusters_or_labels})"
        else:
            title += f"(N_Clusters: {self.n_clusters_or_labels}, Silhouette Score: {self.score:.2f})"
        self.ax.set_title(title)

        self.ax.set_xlabel("Dimension 1")
        self.ax.set_ylabel("Dimension 2")

        # --- Cluster Legend (Top Right, Outside) ---
        ...
        self.ax.legend(handles=legend_handles, title=label_title,
                       bbox_to_anchor=(1.02, 1), loc='upper left', borderaxespad=0)

        # Adjust layout
        self.fig.subplots_adjust(right=0.8, left=0.1, bottom=0.1, top=0.9)

        # --- File Legend (Bottom Scrollable Panel) ---
        for widget in self.legend_inner_frame.winfo_children():
            widget.destroy()

        # Get current theme background so the canvas corners blend in
        try:
            bg_color = tb.Style().lookup("TFrame", "background")
        except:
            bg_color = "#2b3e50"  # Fallback for superhero theme

        for i, file_path in enumerate(unique_files):
            row_frame = tb.Frame(self.legend_inner_frame)
            row_frame.pack(fill="x", pady=2, padx=5)

            # --- MODIFIED: Draw Circular Bubble using Canvas ---
            rgba = file_color_map[file_path]
            hex_color = matplotlib.colors.to_hex(rgba)

            # 22x22 Canvas with theme background
            icon_canvas = tk.Canvas(row_frame, width=22, height=22, bg=bg_color, highlightthickness=0)
            icon_canvas.pack(side="left", padx=(0, 8))

            # Draw filled circle (oval)
            icon_canvas.create_oval(4, 4, 18, 18, fill=hex_color, outline=hex_color)
            # ---------------------------------------------------

            lbl = tb.Label(row_frame, text=os.path.basename(file_path))
            lbl.pack(side="left")
...
class AudioVisualizerApp(tb.Window):
    ...
    def create_controls(self):
        ...
        # --- REPLACED SECTION: File Label + Link Container ---
        # Create a frame to hold the text and the link side-by-side
        self.file_info_frame = tb.Frame(self.scrollable_frame)
        self.file_info_frame.grid(row=row_idx, column=0, sticky="w", pady=(0, 15))

        # The status text (e.g., "13 files loaded")
        self.file_label = tb.Label(self.file_info_frame, text="No file uploaded", justify="left")
        self.file_label.pack(side="left")

        # The link (e.g., "View Files") - initially hidden
        # We manually set foreground='blue' and underline to ensure the look
        self.view_files_link = tk.Label(
            self.file_info_frame,
            text="View Files",
            fg="#3498db",  # Blue color
            font=("Segoe UI", 9, "underline"),
            cursor="hand2",  # Changes cursor to hand on hover
            bg="#2b3e50"  # Match your theme background (Superhero dark gray)
            # If the background looks wrong, remove the 'bg' line or set it to system default
        )
        # Bind the click event
        self.view_files_link.bind("<Button-1>", lambda e: self.show_file_list_window())
        # We don't pack it yet; we will pack it in load_file when needed
        # -----------------------------------------------------
    ...
    def create_display(self):
        ...
        # 1. Plot Frame (Top, expands)
        self.main_plot_frame = tb.Frame(self.display_frame)
        self.main_plot_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5))

        self.fig, self.ax = plt.subplots(figsize=(7, 5))
        self.canvas = FigureCanvasTkAgg(self.fig, master=self.main_plot_frame)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        ...
        # 2. Legend Frame (Bottom, fixed height, scrollable)
        lbl_frame = tb.Frame(self.display_frame)
        lbl_frame.pack(fill="x", pady=(5, 0))
        tb.Label(lbl_frame, text="File Legend:", font=("Segoe UI", 9, "bold")).pack(side="left")

        self.main_legend_frame = tb.Frame(self.display_frame, height=150)
        self.main_legend_frame.pack(fill=tk.X, pady=5)
        self.main_legend_frame.pack_propagate(False)  # Fixed height

        self.main_legend_scroll = tb.Scrollbar(self.main_legend_frame, orient="vertical")
        self.main_legend_scroll.pack(side="right", fill="y")

        self.main_legend_canvas = tk.Canvas(self.main_legend_frame, yscrollcommand=self.main_legend_scroll.set)
        self.main_legend_canvas.pack(side="left", fill="both", expand=True)
        self.main_legend_scroll.config(command=self.main_legend_canvas.yview)

        self.main_legend_inner = tb.Frame(self.main_legend_canvas)
        self.main_legend_canvas.create_window((0, 0), window=self.main_legend_inner, anchor="nw")

        self.main_legend_inner.bind("<Configure>",
                                    lambda e: self.main_legend_canvas.configure(
                                        scrollregion=self.main_legend_canvas.bbox("all")))
    ...
    def load_file(self):
        ...
        if not all(os.path.splitext(fp)[1].lower() == first_file_ext for fp in file_paths):
            ...
            self.view_files_link.pack_forget()  # Hide link
            self.has_labels = False
            self.update_ui_state()
            return
        ...
        # --- MODIFIED SECTION: Text + Link Logic ---
        count = len(self.file_paths)
        if count == 1:
            # If only 1 file, just show the name, no link needed
            self.file_label.configure(text=f"Loaded: {display_names[0]}")
            self.view_files_link.pack_forget()
        else:
            # If multiple files, show count and the "View Files" link
            self.file_label.configure(text=f"{count} files loaded.  ")  # Added spaces for padding
            self.view_files_link.pack(side="left")
            # -------------------------------------------
    ...
    def plot_data(self, reduced, labels, ft, n_clusters_or_labels, segment_len, method, score, has_labels):
        ...
        title = f"{method} Plot\n"
        if has_labels:
            title += f"(N_Labels: {n_clusters_or_labels})"
        else:
            title += f"(N_Clusters: {n_clusters_or_labels}, Silhouette Score: {score:.2f})"
        self.ax.set_title(title)

        self.ax.set_xlabel("Dimension 1")
        self.ax.set_ylabel("Dimension 2")

        # --- Cluster Legend (Outside Right) ---
        ...
        # --- File Legend (Populate Scrollable Panel) ---
        for widget in self.main_legend_inner.winfo_children():
            widget.destroy()
        ...
        for i, file_path in enumerate(unique_files):
            row_frame = tb.Frame(self.main_legend_inner)
            row_frame.pack(fill="x", pady=2, padx=5)
    ...
    def show_file_list_window(self):
        """Opens a popup window listing all loaded files."""
        if not self.file_paths:
            return

        window = tb.Toplevel(self)
        window.title("Loaded Files")
        window.geometry("600x400")

        # Label at top
        tb.Label(window, text=f"Total Files: {len(self.file_paths)}", font=("Segoe UI", 12, "bold")).pack(pady=10)

        # Scrolled Text area to show files
        text_area = scrolledtext.ScrolledText(window, width=70, height=20)
        text_area.pack(fill="both", expand=True, padx=10, pady=10)

        # Insert file names
        for path in self.file_paths:
            text_area.insert(tk.END, os.path.basename(path) + "\n")

        # Make read-only
        text_area.configure(state='disabled')

        # Add a Close button
        tb.Button(window, text="Close", bootstyle="secondary", command=window.destroy).pack(pady=5)
